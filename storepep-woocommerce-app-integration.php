...
if (in_array('woocommerce/woocommerce.php', apply_filters('active_plugins', get_option('active_plugins')))) {
    ...
    class StorePep_App_Integration_Processor {
        ...
        function new_customer_created($customer_id) 
        {
            if ($this->enable && $this->new_customer) {
                ...
                $name = ((wc()->version < '3.0.0')?($customer->first_name.' '.$customer->last_name):($customer->get_first_name().' '.$customer->get_last_name()));
                $username = ((wc()->version < '3.0.0')?$customer->username:$customer->get_username());
                $shop_url = site_url();
    ...
    function storepep_plugin_action_links($links)
    {
        ...
        $plugin_links = array(
            ...
            //'<a href="https://www.xadapter.com/online-support/" target="_blank">' . __('Support', 'storepep') . '</a>'
        );
        return array_merge($plugin_links, $links);
    }
} else {
    add_action('admin_notices', 'storepep_wc_admin_notices', 99);
    function storepep_wc_admin_notices()
    {
        is_admin() && add_filter('gettext', function($translated_text, $untranslated_text, $domain)
        {
            ...
            $new = "<span style='color:red'>StorePep WooCommerce App Integration - Plugin Needs Woocommerce to Work";
